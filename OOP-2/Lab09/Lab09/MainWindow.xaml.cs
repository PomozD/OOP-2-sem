...
namespace Lab09
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            Actor = new Actor();
            var item = from Actor in db.actors select Actor;
            comboSort.ItemsSource = item.ToList();
            comboSort.DisplayMemberPath = "Role";
            comboSort.SelectedValuePath = "Id";
        ...
        public static async Task FindObject() { // определение асинхронного метода //Возвращаем объект Task
            using (ActorContext db = new ActorContext())
            {
                string str = "";
                if (role.Text.Length != 0)
                {
                    try
                    {
                        ...
                        int id = actor.Id;
                        var a = await db.actors.FindAsync(id); // вызов асинхронного метода
        ...
        {
            var comboSorted = comboSort.Text;
            db = new ActorContext();

            var data = db.actors.Where(b => b.Role == comboSorted);
            dgSort.ItemsSource = data.ToList();
        }
            ...
            using(ActorContext db = new ActorContext())
            {
                using(var transaction = db.Database.BeginTransaction())
                {
                    try {
                        ...
                        var query2 = db.Database.SqlQuery<User>("select * from TEST;");
                        for (int i = 0; i < query2.Count(); i++)
                        {
                            str += query2.ElementAt(i).Id + "\t" +
                                query2.ElementAt(i).Name + "\t\n";
                        }
                        MessageBox.Show(str);
                        db.SaveChanges();
                        Script.Text = "commit";
